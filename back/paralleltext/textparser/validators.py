...
def validate_text_file(file_buffer):
    """
    Validate text file.
    A valid text file is either of type ASCII text or Unicode text
    A valid text file must not be of type executable, shell or script
    """
    ...
    if any(valid_type in mime_type for valid_type in allowed_mime_type_elements) \
        and not(any(forbidden_type in mime_type
                    for forbidden_type in forbidden_mime_type_elements)):
        return file_buffer
